...
2.  **Configure the Application**
    ...
    **Variables defined in `config.txt`:** You can check
    ...
    `IMAGE_MODEL`          | Image model for outpainting and image generation.          | `gemini-3.1-pro-image` (Nano Banana Pro), `gemini-3.1-flash-image` (Nano Banana 2)
    `IMAGE_REGION`         | Region for image model invocation.                         | Check availability. Recommended `global`.
